...
def process_sample(paper, bioproject, sample):
    ...
    kraken_unclassified_read_ids = set()
    for kraken_output in glob.glob("*.kraken2.tsv.gz"):
        with gzip.open(kraken_output, "rt") as inf:
            for line in inf:
                if line.startswith("U"):
                    kraken_unclassified_read_ids.add(line.split("\t")[1])
    
    als = defaultdict(list)
    als_fname = os.path.join("%s.alignments.masked-41.tsv" % sample)
    ...
    for read_id in sorted(als, key=lambda k: hashlib.sha256(k.encode('utf-8')).digest()):
        if read_id not in kraken_unclassified_read_ids:
            continue
