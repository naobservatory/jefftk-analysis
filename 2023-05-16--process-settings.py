...
for settings in glob.glob("*.settings"):
    ...
    # Happen to know that each site used consistent methods.
    site_method[site] = sample_metadata[sample]["method"]
...
for site in sorted(discard_fraction_by_date_and_site):
    ...
    ax.scatter(xs, ys, label="%s (%s)" % (site, site_method[site]))
ax.legend()
...
for site in sorted(by_site):
    ...
    ax.plot(xs, ys, label="%s (%s)" % (site, site_method[site]))
ax.legend()
