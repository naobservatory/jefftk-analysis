...
methods = set(metadata_samples[sample]["method"]
              for sample in samples)
markers=("d", "v", "s", "*", "^")

for i, taxid in enumerate(sorted(target_cc_taxids)):
    fig, ax = plt.subplots(constrained_layout=True)
    plt.suptitle("relative abundances by sampling method")
    ...
    for method_index, method in enumerate(sorted(methods)):
        ...
        ys = []
        for sample in samples:
            if metadata_samples[sample]["method"] == method:
        ...
        ax.scatter(xs, ys, label=method, marker=markers[method_index])
    ax.legend()
    
    fig.savefig("smc-%s.png" % ( target_cc_taxids[taxid].replace(" ", "-")),
                dpi=180)
    plt.clf()
